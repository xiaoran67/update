...
# ==================== 配置常量 ====================
CONFIG = {
    'input_base': 'scripts/livesource3',
    'output_base': 'output/livesource3',
    
    'output_files': {
        'full': 'output/livesource3/full.txt',
        'lite': 'output/livesource3/lite.txt', 
        'custom': 'output/livesource3/custom.txt',
        'others': 'output/livesource3/others.txt'
    },
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"{CONFIG['output_base']}/processing.log", encoding='utf-8'),
...
def read_txt_to_array(file_name):
    ...
    try:
        ...
        logger.error(f"读取文件错误 {file_name}: {e}")
        return []
...
def read_blacklist_from_txt(file_path):
    ...
    try:
        ...
        logger.error(f"读取黑名单错误 {file_path}: {e}")
        return []
...
# ==================== 黑名单处理 ====================
logger.info("加载黑名单...")
blacklist_auto = read_blacklist_from_txt(f"{CONFIG['input_base']}/blacklist/blacklist_auto.txt") 
blacklist_manual = read_blacklist_from_txt(f"{CONFIG['input_base']}/blacklist/blacklist_manual.txt") 
combined_blacklist = set(blacklist_auto + blacklist_manual)
logger.info(f"黑名单加载完成: {len(combined_blacklist)} 条记录")
...
def process_name_string(input_str):
    ...
    try:
        ...
        logger.error(f"处理频道名称错误: {e}, 输入: {input_str}")
        return input_str
...
def get_http_response(url, timeout=None, retries=None, backoff_factor=None):
    ...
    for attempt in range(retries):
        try:
            ...
            logger.error(f"[HTTP错误] 代码: {e.code}, URL: {url}")
            break
        except urllib.error.URLError as e:
            logger.warning(f"[URL错误] 原因: {e.reason}, 尝试: {attempt + 1}/{retries}")
        except socket.timeout:
            logger.warning(f"[超时] URL: {url}, 尝试: {attempt + 1}/{retries}")
        except Exception as e:
            logger.warning(f"[异常] {type(e).__name__}: {e}, 尝试: {attempt + 1}/{retries}")
...
def process_channel_line(line):
    ...
    try:
        ...
        logger.error(f"处理频道行错误: {e}, 行内容: {line}")
        ...
        for line in lines:
            if ("#genre#" not in line and "," in line and "://" in line and 
                ...
                try:
                    ...
                    logger.warning(f"处理行错误: {e}, 行: {line}")

        logger.info(f"处理完成: {valid_lines} 个有效频道")
...
def load_corrections_name(filename):
    ...
    try:
        ...
        logger.info(f"纠错数据加载: {len(corrections)} 条规则")
    except Exception as e:
        logger.error(f"加载纠错数据错误 {filename}: {e}")
    return corrections
...
def correct_name_data(corrections, data):
    ...
    for line in data:
        try:
            ...
            logger.warning(f"纠正名称错误: {e}, 行: {line}")
...
def sort_data(order, data):
    ...
    try:
        ...
        logger.warning(f"排序数据错误: {e}")
        return data

# ==================== 加载字典数据 ====================
logger.info("加载字典数据...")
# 主频道字典
ys_dictionary = read_txt_to_array(f"{CONFIG['input_base']}/主频道/CCTV.txt")
ws_dictionary = read_txt_to_array(f"{CONFIG['input_base']}/主频道/卫视频道.txt")
tyss_dictionary = read_txt_to_array(f"{CONFIG['input_base']}/主频道/体育赛事.txt")

# 地方台字典
hb_dictionary = read_txt_to_array(f"{CONFIG['input_base']}/地方台/湖北频道.txt")
hn_dictionary = read_txt_to_array(f"{CONFIG['input_base']}/地方台/湖南频道.txt")

# 加载纠错数据
corrections_name = load_corrections_name(f"{CONFIG['input_base']}/corrections_name.txt")

logger.info(f"字典数据加载完成: CCTV({len(ys_dictionary)}) 卫视({len(ws_dictionary)}) 湖北({len(hb_dictionary)}) 湖南({len(hn_dictionary)})")
...
def main():
    ...
    # 1. 处理URL源
    urls = read_txt_to_array(f"{CONFIG['input_base']}/urls-daily.txt")
    logger.info(f"发现 {len(urls)} 个URL源")
    ...
    # 2. 处理白名单
    logger.info("处理白名单...")
    whitelist_auto_lines = read_txt_to_array(f"{CONFIG['input_base']}/blacklist/whitelist_auto.txt")
    whitelist_count = 0
    for whitelist_line in whitelist_auto_lines:
        if ("#genre#" not in whitelist_line and "," in whitelist_line and 
            ...
            if response_time < 2000:  # 2秒以内的高响应源
                ...
                whitelist_count += 1
    logger.info(f"白名单处理完成: {whitelist_count} 个高速源")

    # 3. 处理手工区
    logger.info("处理手工区...")
    manual_channels = read_txt_to_array(f"{CONFIG['input_base']}/手工区/手工频道.txt")
    for channel in manual_channels:
        process_channel_line(channel)
    
    logger.info(f"手工区处理完成: {len(manual_channels)} 个手工频道")
    ...
    if aktv_text:
        logger.info("AKTV成功获取内容")
        aktv_text = convert_m3u_to_txt(aktv_text)
        aktv_lines.extend(aktv_text.strip().split('\n'))
    else:
        logger.warning("AKTV请求失败，从本地获取！")
        aktv_lines.extend(read_txt_to_array(f"{CONFIG['input_base']}/手工区/AKTV.txt"))
    logger.info(f"AKTV处理完成: {len(aktv_lines)} 个频道")
    ...
    def normalize_date_to_md(text):
        ...
        try:
            ...
            logger.warning(f"日期格式化错误: {e}, 文本: {text}")
            return text
    ...
    version = formatted_time + "," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推台.txt")
    about = "livesource3," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推台.txt")
    daily_mtv = "今日推荐," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推荐.txt")
    daily_mtv1 = "🔥低调," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推荐.txt")
    daily_mtv2 = "🔥使用," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推荐.txt")
    daily_mtv3 = "🔥禁止," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推荐.txt")
    daily_mtv4 = "🔥贩卖," + get_random_url(f"{CONFIG['input_base']}/手工区/今日推荐.txt")
    ...
    all_lines_full.extend(["☘️湖南频道,#genre#"] + sort_data(hn_dictionary, set(correct_name_data(corrections_name, hn_lines))) + ['\n'])
    all_lines_full.extend(["🏆体育赛事,#genre#"] + normalized_tyss_lines + ['\n'])
    all_lines_full.extend(["🚀AKTV📶,#genre#"] + aktv_lines + ['\n'])
    all_lines_full.extend(["🕒更新时间,#genre#"] + [version, about, daily_mtv, daily_mtv1, daily_mtv2, daily_mtv3, daily_mtv4] + read_txt_to_array(f"{CONFIG['input_base']}/手工区/about.txt") + ['\n'])
    ...
    all_lines_lite.extend(["地方频道,#genre#"] + sort_data(hb_dictionary, set(correct_name_data(corrections_name, hb_lines))) + sort_data(hn_dictionary, set(correct_name_data(corrections_name, hn_lines))) + ['\n'])
    all_lines_lite.extend(["体育赛事,#genre#"] + normalized_tyss_lines + ['\n'])
    all_lines_lite.extend(["AKTV,#genre#"] + aktv_lines + ['\n'])
    ...
    all_lines_custom.extend(["🏠地方频道,#genre#"] + sort_data(hb_dictionary, set(correct_name_data(corrections_name, hb_lines))) + sort_data(hn_dictionary, set(correct_name_data(corrections_name, hn_lines))) + ['\n'])
    all_lines_custom.extend(["🏆体育赛事,#genre#"] + normalized_tyss_lines + ['\n'])
    all_lines_custom.extend(["🚀AKTV📶,#genre#"] + aktv_lines + ['\n'])
    all_lines_custom.extend(["🕒更新时间,#genre#"] + [version, about, daily_mtv, daily_mtv1, daily_mtv2, daily_mtv3, daily_mtv4] + read_txt_to_array(f"{CONFIG['input_base']}/手工区/about.txt") + ['\n'])
    ...
    for file_type, lines in output_data.items():
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write('\n'.join(lines))
            logger.info(f"{file_type}源已保存: {file_path}")
            
            if os.path.exists(file_path):
                file_size = os.path.getsize(file_path)
                lines_count = len(lines)
                logger.info(f"  大小: {file_size} 字节, 行数: {lines_count}")
                
        except Exception as e:
            logger.error(f"保存{file_type}文件时发生错误：{e}")

    # 8. 生成HTML文件
    def generate_playlist_html(data_list, output_file=f"{CONFIG['output_base']}/sports.html"):
        """生成HTML播放列表"""
        try:
            html_head = '''
            <!DOCTYPE html>
            ...
            <head>
                ...
                <title>最新体育赛事 - LiveSource3</title>
                <style>
            ...
            logger.error(f"生成HTML文件时发生错误：{e}")

    # 生成体育赛事HTML
    logger.info(f"体育赛事数据行数: {len(normalized_tyss_lines)}")
    if normalized_tyss_lines and len(normalized_tyss_lines) > 0:
        valid_sports_data = [entry for entry in normalized_tyss_lines if ',' in entry and len(entry.strip()) > 0]
        if valid_sports_data:
            generate_playlist_html(sorted(set(valid_sports_data)))
        else:
            logger.info("没有有效的体育赛事数据，跳过生成HTML文件")
    else:
        logger.info("没有体育赛事数据，跳过生成HTML文件")

    # 9. 生成M3U文件
    def get_logo_by_channel_name(channel_name):
        """根据频道名称获取logo"""
        try:
            channels_logos = read_txt_to_array(f"{CONFIG['input_base']}/logo.txt")
            ...
            logger.warning(f"获取logo时发生错误：{e}")
        return None
    ...
    def make_m3u(txt_file, m3u_file):
        ...
        try:
            if not os.path.exists(txt_file):
                logger.error(f"TXT文件不存在: {txt_file}")
                return
            ...
            logger.info(f"M3U文件生成: {m3u_file}")
        except Exception as e:
            logger.error(f"生成M3U文件时发生错误：{e}")

    # 为所有版本生成对应的M3U文件
    logger.info("生成M3U文件...")
    for file_type in ['full', 'lite', 'custom', 'others']:
        make_m3u(CONFIG['output_files'][file_type], CONFIG['output_files'][file_type].replace(".txt", ".m3u"))
    ...
    logger.info("\n=============== 执行统计 ===============")
    logger.info(f"开始时间: {timestart.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"结束时间: {timeend.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"执行时间: {minutes}分{seconds}秒")
    logger.info(f"黑名单: {len(combined_blacklist)} 条")
    logger.info(f"白名单: {len(whitelist_auto_lines)} 条")
    logger.info(f"央视源: {len(ys_lines)} 个")
    logger.info(f"卫视源: {len(ws_lines)} 个") 
    logger.info(f"湖北源: {len(hb_lines)} 个")
    logger.info(f"湖南源: {len(hn_lines)} 个")
    logger.info(f"体育赛事: {len(normalized_tyss_lines)} 个")
    logger.info(f"AKTV: {len(aktv_lines)} 个")
    logger.info(f"其它源: {len(others_lines)} 个")
    logger.info(f"全部源: {len(all_lines_full)} 行")
    logger.info(f"精简源: {len(all_lines_lite)} 行")
    logger.info(f"定制源: {len(all_lines_custom)} 行")
    logger.info("========================================")
    ...
    for file_type, file_path in CONFIG['output_files'].items():
        if os.path.exists(file_path):
            ...
            logger.info(f"  {status} {file_type}: {file_path} ({file_size} 字节)")
            if file_size == 0:
                all_files_ok = False
        else:
            logger.info(f"  ❌ {file_type}: {file_path} (文件不存在)")
            all_files_ok = False
    ...
    for file_type in ['full', 'lite', 'custom', 'others']:
        ...
        if os.path.exists(m3u_file):
            ...
            status = "✅" if file_size > 0 else "❌"
            logger.info(f"  {status} {file_type}.m3u: {m3u_file} ({file_size} 字节)")
        else:
            logger.info(f"  ❌ {file_type}.m3u: {m3u_file} (文件不存在)")
            all_files_ok = False

    if all_files_ok:
        logger.info("🎉 所有文件生成成功！")
    else:
        logger.warning("⚠️ 部分文件生成有问题，请检查！")
