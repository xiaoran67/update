...
Open https://github.com/xiaoran67/iptv-api and click `Star` to favorite this project (Your Star is my motivation for
continuous updates).
...
1. Download the [IPTV-API Update Software](https://github.com/xiaoran67/iptv-api/releases), open the software, and click
...
```bash
docker pull xiaoran67/iptv-api:latest
```

🚀 Proxy acceleration (recommended for users in China, may be cached):

```bash
docker pull docker.1ms.run/xiaoran67/iptv-api:latest
```

#### 2. Run the container

```bash
docker run -d -p 5180:5180 xiaoran67/iptv-api
```
